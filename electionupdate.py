...
def showelectionscheduleupdate():
    ...
    def finddata():
        ...
        sql="select wardname,areacode,areaname,dateofelection from electionschedule where wardno=%d"%(xa)
        cur.execute(sql)
        e2.delete(0,100)
        e3.delete(0,100)
        e4.delete(0,100)
    ...
    def updatedata():
        ...
        xd=e4.get()
        xe=e5.get()
        sql="Update electionschedule set wardname='%s',areacode='%s',areaname='%s',dateofelection='%s' where wardno=%d"%(xb,xc,xd,xe,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    b=Label(t,text='Ward Name',font=("Arial",15),bg='light blue')
    b.place(x=75,y=200)
    e2=Entry(t,width='25')
    e2.place(x=250,y=200)
    
    c=Label(t,text='Area Code',font=("Arial",15),bg='light blue')
    c.place(x=75,y=250)
    e3=Entry(t,width='25')

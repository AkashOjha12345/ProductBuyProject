...
def showsareasavedata():
    ...
    def savedata():
        if len(e1.get())==0 or len(e2.get())==0 or len(e3.get())==0 or len(e4.get())==0 or len(e5.get())==0 :
            ...
            sql="insert into area values('%s','%s','%s','%s','%s')"%(xa,xb,xc,xd,xe)
            cur.execute(sql)
            db.commit()
            db.close()
    ...
    a=Label(t,text='AREA',font='55',fg='white',bg='black')
    a.place(x=250,y=50)
    
    b=Label(t,text='Area Code',font=("Arial",15),borderwidth=2,bg='light blue')
    b.place(x=70,y=125)
    e1=Entry(t,width='25')
    e1.place(x=400,y=125)
    
    d=Label(t,text='Area Name',font=("Arial",15),bg='light blue')
    d.place(x=70,y=200)
    e2=Entry(t,width='25')
    e2.place(x=400,y=200)
    
    
    e=Label(t,text='City',font=("Arial",15),bg='light blue')
    e.place(x=70,y=275)
    e3=Entry(t,width='25')
